...
Teste toutes les combinaisons de contexte avec des questions réalistes
"""
...
def test_league_context():
    ...
    tests = [
        ...
        {
            "name": "Statistiques générales La Liga",
            "question": "Quelles sont les équipes en forme en ce moment ?",
            "context": {
                "context_type": "league",
                "league_id": 140,
                "league_name": "La Liga",
...
def test_match_context():
    ...
    if not fixtures:
        print("⚠️  No fixtures found for Ligue 1")
        return []
    ...
    tests = [
        {
            "name": "Analyse match",
            "question": f"Analyse complète du match {home_team.get('name')} vs {away_team.get('name')}",
            "context": {
                "context_type": "match",
                "league_id": 61,
                "match_id": match_id,
        ...
        },
        {
            ...
            "question": "Quel est l'historique des confrontations entre ces deux équipes ?",
            "context": {
                "context_type": "match",
                "league_id": 61,
                "match_id": match_id,
...
def test_player_match_context():
    ...
    # Using known player IDs (Mbappé, etc.)
    tests = [
        {
            "name": "Performance Mbappé dans match",
            "question": "Comment a joué Mbappé dans ce match ?",
            "context": {
                "context_type": "player",
                "league_id": 61,
    ...
    for test in tests:
        print(f"\n🧪 Test: {test['name']}")
        print(f"   Question: {test['question']}")
        print(f"   ⚠️  Note: May fail if match_id doesn't exist - testing workflow validation")
...
def test_player_team_context():
    ...
    tests = [
        ...
        {
            "name": "Forme Mbappé saison",
            "question": "Quelle est la forme de Mbappé cette saison ?",
            "context": {
                "context_type": "player",
                "league_id": 61,
                "team_id": 85,  # PSG
...
| **✅ Réussis** | {successful} ({successful/total_tests*100:.1f}%) |
| **❌ Échoués** | {failed} ({failed/total_tests*100:.1f}%) |
    ...
    for context_type, results in by_context.items():
        ...
        for result in results:
            ...
            if result.success:
                ...
                report_md += f"**Tools Appelés** ({len(result.tools_called)}): `{', '.join(result.tools_called)}`\n\n"
